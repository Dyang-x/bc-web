...
import pako from 'zpl-image/pako';
import { imageToZ64 } from "./imageToZ64";
import {  Divider  } from '@hvisions/h-ui';

export default function ModalQR({value,setPrintDatas}) {

  const qrcodeRef = useRef();

	useEffect(() => {
	// 	// const text = 'Hello World!';
    // const text = value[0].code
	// 	// qrcodeRef.current.innerHTML = '';
	// 	document.getElementById(value[0].code).innerHTML = '';
	// 	//qrcode.clear(); 
	// 	var qrcode = new QRCode(value[0].code, {
	// 		text: text, 
	// 		width: 380,
	// 		height: 380,
	// 		// render: 'canvas'
...
// 	const mono = monochrome(rgba, width, height, +opts.black || 50, opts.notrim);
...
// 			const r = rgba[i] * .3 * a + 255 * (1 - a);
// 			const g = rgba[i+1] * .59 * a + 255 * (1 - a);
// 			const b = rgba[i+2] * .11 * a + 255 * (1 - a);
// 			const gray = r + g + b;
...
// 	const buf	 = new Uint8Array(~~((height + 7) / 8) * width);
...
// 	const buf	 = new Uint8Array(~~((width + 7) / 8) * height);
// 	let idx	 = 0;	// index into buf
// 	let byte = 0;	// current byte of image data
// 	let bitx = 0;	// bit index
			  ...
			  {value.map(i => {
				  return (
					  <>
						  ...
						  <div key={`div2${i.code}`}style={{ marginLeft:240 }}>{i.code}</div>
						  <Divider key={`divider${i.code}`} type="horizontal" style={{margin:'10px 0 0 0'}} />,
					  </>
				  )
			  })}
