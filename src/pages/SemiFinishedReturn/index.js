...
const SemiFinishedReturn = ({ history }) => {
  ...
  const columns = [
    {
      title: getFormattedMsg('SemiFinishedReturn.title.receiptNumber'),
      dataIndex: 'receiptNumber',
      key: 'receiptNumber',
      align: 'center'
    },
    {
      title: getFormattedMsg('SemiFinishedReturn.title.createTime'),
      dataIndex: 'createTime',
      key: 'createTime',
      align: 'center'
    },
    {
      title: getFormattedMsg('SemiFinishedReturn.title.creator'),
      dataIndex: 'creator',
      key: 'creator',
      align: 'center'
    },
    {
      title: getFormattedMsg('SemiFinishedReturn.title.updateTime'),
      dataIndex: 'updateTime',
      key: 'updateTime',
      align: 'center'
    },
    {
      title: getFormattedMsg('SemiFinishedReturn.title.updateCreator'),
      dataIndex: 'updateCreator',
      key: 'updateCreator',
      align: 'center'
    },
    {
      title: getFormattedMsg('SemiFinishedReturn.title.trayNumber'),
      dataIndex: 'trayNumber',
      key: 'trayNumber',
      align: 'center'
    },
    {
      title: getFormattedMsg('SemiFinishedReturn.title.orderCount'),
    ...
    {
      title: getFormattedMsg('SemiFinishedReturn.title.attributeThree'),
    ...
    {
      title: getFormattedMsg('SemiFinishedReturn.title.dockingPoint'),
    ...
    {
      title: getFormattedMsg('SemiFinishedReturn.title.sortPosition'),
    ...
    {
      title: getFormattedMsg('SemiFinishedReturn.title.operation'),
      key: 'opt',
      align: 'center',
      width: 200,
      render: (_, record) => [
        <a key="update" onClick={() => handleUpdate(record)}>
          {getFormattedMsg('SemiFinishedReturn.button.update')}
        </a>,
        ...
        >
          {getFormattedMsg('SemiFinishedReturn.button.delete')}
        </a>,
        <Divider key="divider2" type="vertical" />,
        <a key="warehousing" type="primary" onClick={() => handleWarehousing(record)}>
          {getFormattedMsg('SemiFinishedReturn.button.warehousing')}
        </a>
      ]
    }
  ];
  ...
  const modalAddFoot = () => [
    <Button key="save" type="primary" onClick={handleSaveAdd}>
      {getFormattedMsg('SemiFinishedReturn.button.save')}
    </Button>,
    <Button key="cancel" onClick={handleCancelAdd}>
      {getFormattedMsg('SemiFinishedReturn.button.cancel')}
    </Button>
  ];
  ...
  const handleSaveAdd = () => {
    ...
    validateFields(async (err, values) => {
      ...
      //       message: getFormattedMsg('SemiFinishedReturn.message.addSuccess')
      //     });
      //     loadData(page, pageSize, { ...searchValue, state: selectedstatus });
      //   })
      //   .catch(err => {
      //     notification.warning({
      //       message: getFormattedMsg('SemiFinishedReturn.message.addFailure'),
      //       description: err.message
      //     });
      //   });
      handleCancelAdd();
  ...
  const HandleSaveUpdate = () => {
    ...
    validateFields(async (err, values) => {
      ...
      //   .then(res => {
      //     notification.success({
      //       message: getFormattedMsg('SemiFinishedReturn.message.updateSuccess'),
      //     });
      //     loadData(page, pageSize, { ...searchValue, state: selectedstatus });
      //   })
      //   .catch(err => {
      //     notification.warning({
      //       message: getFormattedMsg('SemiFinishedReturn.message.updateFailure'),
      //       description: err.message
      //     });
      //   });
      handleCancelUpdate();
  ...
  const handleDelete = record => {
    Modal.confirm({
      title: getFormattedMsg('SemiFinishedReturn.operation.delete'),
      okType: 'danger',
      onOk: async () => {
        notification.warning({
          message: '没有接口'
        ...
        //   .then(res => {
        //     notification.success({
        //       message: getFormattedMsg('SemiFinishedReturn.message.deleteSuccess'),
        //     });
        //     loadData(page, pageSize, { ...searchValue, state: selectedstatus });
        //   })
        //   .catch(err => {
        //     notification.warning({
        //       message: getFormattedMsg('SemiFinishedReturn.message.deleteFailure'),
        //       description: err.message
        //     });
        //   });
      },
  ...
  const handleWarehousing = record => {
    Modal.confirm({
      title: getFormattedMsg('SemiFinishedReturn.message.warehousing'),
      onOk: async () => {
        notification.warning({
          message: '没有接口'
        });
        ...
        //   .then(res => {
        //     notification.success({
        //       message: getFormattedMsg('SemiFinishedReturn.message.warehousingSuccess'),
        //     });
        //     loadData(page, pageSize, { ...searchValue, state: selectedstatus });
        //   })
        //   .catch(err => {
        //     notification.warning({
        //       message: getFormattedMsg('SemiFinishedReturn.message.warehousingFailure'),
        //       description: err.message
        //     });
        //   });
      },
  ...
  return (
    <>
      <HVLayout>
        <HVLayout.Pane height={'auto'}>
          <SearchForm onSearch={handleSearch}>
            <SearchForm.Item
              label={getFormattedMsg('SemiFinishedReturn.label.receiptNumber')}
              name="receiptNumber"
            >
              <Input
                placeholder={getFormattedMsg('SemiFinishedReturn.placeholder.receiptNumber')}
                allowClear
              />
            </SearchForm.Item>
            <SearchForm.Item
              label={getFormattedMsg('SemiFinishedReturn.label.trayNumber')}
              name="trayNumber"
            >
              <Input
                placeholder={getFormattedMsg('SemiFinishedReturn.placeholder.trayNumber')}
                allowClear
              />
            </SearchForm.Item>
            <SearchForm.Item
              label={getFormattedMsg('SemiFinishedReturn.label.creationTime')}
              name="creationTime"
            >
              <RangePicker
                placeholder={getFormattedMsg('SemiFinishedReturn.placeholder.creationTime')}
                format={dateTime}
                showTime
                style={{ width: '100%' }}
              />
        ...
        <HVLayout.Pane
          ...
          title={getFormattedMsg('SemiFinishedReturn.title.tableName')}
          buttons={[
            <Button key="add" type="primary" onClick={() => handleAdd()}>
              {getFormattedMsg('SemiFinishedReturn.button.add')}
            </Button>
          ]}
      ...
      <Drawer
        title={getFormattedMsg('SemiFinishedReturn.title.addOrder')}
        visible={addVis}
      ...
      <Drawer
        title={getFormattedMsg('SemiFinishedReturn.button.update')}
        visible={updateVis}
...
export default SemiFinishedReturn;
